...
class WebUiE2ETests(unittest.TestCase):
    ...
    def test_webui_capture_preview_accept_export_and_stop(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                home = self.get(url, "/")
                self.assertIn("Capture clipboard &amp; generate", home)
                self.assertIn("Add study material", home)
                self.assertIn('name="text"', home)
                ...
                self.assertIn("New cards", home)
                self.assertIn("All cards", home)
                self.assertIn("All stacks", home)
                self.assertIn('class="stop-action"', home)
                self.assertNotIn(">Rebuild all<", home)
