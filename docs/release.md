...
The one-folder build includes `autoanki/system_prompt.txt`, so the executable can generate cards without needing the source tree. Runtime cards and decks are written beside the executable under `data/` and `output/`.
